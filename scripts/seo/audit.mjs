 ...
 *
 * Usage: npm run seo:audit [-- --dist <dir>] [--no-dist] [--json]
 * Exit 1 on errors, 0 on warnings only. BASE_PATH is honoured when mapping sitemap URLs to dist.
 */
...
function parseArgs(argv) {
  /** @type {{ dist: string, noDist: boolean, json: boolean }} */
  const out = { dist: 'dist', noDist: false, json: false };
...
if (!args.noDist && existsSync(distDir)) {
  issues.push(...auditDist(distDir, { base: process.env.BASE_PATH }));
  distChecked = true;
}
