...
export const landing = {
  ...
  // покупателю вместо стены показывается инструкция и человек, которому можно написать.
  checkoutTitle: 'Как оплатить не из России',
  checkoutLead:
    'Наша касса работает в рублях и только по-русски. Пока это так — вот обходной путь: две минуты, и каждое сообщение мы читаем.',
  ...
  checkoutStep2: 'Напиши нам',
  checkoutStep2Body:
    'Сообщением с той же почтой. На этом пути доступ открывается руками — обычно в течение нескольких часов.',
  checkoutStep3: 'Открой приложение',
  checkoutStep3Body:
    'Перезагрузи — курс или подписка уже там. Если нет, напиши ещё раз: спросить дважды здесь совершенно нормально.',
