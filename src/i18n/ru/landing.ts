...
export const landing = {
  ...
  navLanguage: 'Язык',
  /*
   * Оплата для тех, у кого страницы оплаты нет: касса русская и в рублях, поэтому англоязычному
   * покупателю вместо стены показывается инструкция и человек.
   *
   * Владелец открывает доступ руками, и у неё это занимает минут пять, — поэтому обещан час, её
   * же словами и с запасом. Чего страница делать не должна, так это обещать больше, чем умеет
   * продукт: сообщение в боте настоящее (`purchases_notify` срабатывает на изменение строки, кто
   * бы её ни изменил, — 0027 так и написано), а письма после покупки не существует, и потому его
   * здесь нет. У фразы про бота своё условие: покупатель, пришедший с сайта, телеграм не
   * привязывал и иначе ждал бы сообщения, которое не придёт.
   */
  checkoutTitle: 'Как оплатить не из России',
  checkoutLead:
    'Наша касса работает в рублях и только по-русски. Пока это так — вот обходной путь, и на этом конце человек, а не очередь.',
  checkoutStep1: 'Сделай перевод',
  checkoutStep1Body:
    'По ссылке ниже. В комментарии к переводу укажи почту, с которой регистрировался: именно она открывает доступ, больше платёж ничем не опознать.',
  checkoutStep2: 'Доступ откроется в течение часа',
  checkoutStep2Body:
    'Перевод виден нам сразу, а открыть доступ — минут пять, так что обычно гораздо быстрее. Если ты заходил через наш телеграм-бот, он напишет, как только всё готово.',
  checkoutStep3: 'Открой приложение',
  checkoutStep3Body:
    'Перезагрузи — курс или подписка уже там. Если час прошёл, а их нет, напиши нам: спросить дважды здесь совершенно нормально.',
  checkoutPaypal: 'Оплатить через PayPal',
  checkoutWrite: 'Написать нам',
  checkoutNoPaypal: 'Реквизиты пока не опубликованы. Напиши нам — пришлём их и откроем доступ.',
  checkoutNoHelp: 'Напиши на {email}, и мы поможем.',
  checkoutBackToCourses: 'Вернуться к курсам',
  ...
  subscribeFaq1Q: 'А если я захочу перестать?',
  subscribeFaq1A:
    'Просто не продлевай. Деньги списываются только когда ты сам платишь, курсы открыты до конца оплаченного периода, и никакой отмены оформлять не надо.',
  ...
  subscribeFaq3A:
    'Купленный курс твой навсегда, с подпиской или без. Подписка открывает все остальные курсы Forma — и те, что выйдут позже.',
