...
| robots.txt               | `src/pages/robots.txt.ts`                | `Allow: /`, `Disallow: <base>/app/`, `Sitemap:`, `Host:`. Only effective at the origin root, which is where Cloudflare Pages serves it. On a GitHub Pages _project_ site (base path) crawlers do not read it; use a custom domain or accept it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| OG images                | `scripts/seo/og.mjs` → `public/og/*.png` | 1200×630, dark card with a flat square of the course's tile colour: `default.png`, `hub-<home\|courses\|exercises\|guides>-<locale>.png`, `course-<id>-<locale>.png`, `exercise-<id>-<locale>.png`, `guide-<translationKey>-<locale>.png`. `public/og/` is git-ignored; the deploy workflow runs `npm run seo:og` before `npm run build`. Fonts (Manrope SemiBold for headlines and ExtraBold for the wordmark — resvg picks a face by weight and cannot interpolate — plus Onest Regular) are bundled under the OFL in `scripts/seo/fonts/` and passed to resvg with `loadSystemFonts: false`; if the folder is emptied the script warns and falls back to system fonts. Content and i18n are imported straight from the TypeScript sources (Node ≥ 22.13 type stripping via `scripts/seo/ts-loader.mjs`). The card used to carry a drawn stick figure on that tile; it does not any more. |
